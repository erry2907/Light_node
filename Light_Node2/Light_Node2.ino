...
#define wifi_password "YOURpw"
#define mqtt_server "192.168.0.60"
#define mqtt_port 1883
#define mqtt_user "YOURuser" 
#define mqtt_password "YOURpw2"



/************* MQTT TOPICS (change these topics as you wish)  **************************/
#define light_state_topic "ESP01/lightnode2"
#define light_set_topic "ESP01/lightnode2/set"
...
void setup_wifi() {
  ...
  while (WiFi.status() != WL_CONNECTED) {
    colorWipe(strip.Color(0, 0, 255), 50); // Wipe Blue
    colorWipe(strip.Color(0, 0, 0), 50); // Wipe Off
    //delay(500);
    Serial.print(".");
    
  }
  colorWipe(strip.Color(0, 255, 0), 25); // Wipe green
  colorWipe(strip.Color(0, 0, 0), 25); // Wipe Off
  colorWipe(strip.Color(0, 255, 0), 25); // Wipe green
  colorWipe(strip.Color(0, 0, 0), 25); // Wipe Off
  Serial.println("");
...
//  analogWrite(bluePin, inB);
  ...
  colorStrip(strip.Color(inR, inG, inB)); // Red
...
/********************************** START HANDLE WEBPAGE **********************************/

void handleRoot() {
  
  String s_red = server.arg(0);          // read RGB arguments
  String s_green = server.arg(1);
  String s_blue = server.arg(2);

  red = map(s_red.toInt(), 1024, 0, 0, 255);
  green = map(s_green.toInt(), 1024, 0, 0, 255);
  blue = map(s_blue.toInt(), 1024, 0, 0, 255);
  
  Serial.print("Red: ");
  Serial.print(red);   // for TESTING
  Serial.print(", Green: ");
  Serial.print(green); // for TESTING
  Serial.print(", Blue: ");
  ...
  //colorStrip(strip.Color(red, green, blue)); // Red
...
void handleNotFound(){
  //digitalWrite(led, 1);
  colorStrip(strip.Color(100, 0, 0)); // Color all LED's RED
